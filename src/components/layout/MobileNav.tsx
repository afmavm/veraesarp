...
export default function MobileNav() {
  ...
  const navItems = [
    ...
    { label: 'Arama', href: '/arama', icon: Search, isSearchTrigger: true },
  ...
  return (
    <>
            ...
            if (item.isSearchTrigger) {
              return (
                <button
                  ...
                  className="flex flex-col items-center gap-1 transition-colors relative text-[#5A5652] hover:text-[#B49A6A]"
                >
                  <Icon className="w-5 h-5 text-[#B49A6A]" />
                  <span className="text-[10px] font-semibold text-[#B49A6A] tracking-tight">{item.label}</span>
                </button>
              );
            }
      ...
      {/* Advanced Search Modal for Mobile Nav */}
